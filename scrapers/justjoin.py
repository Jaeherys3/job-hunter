...
def fetch_justjoin() -> list:
    ...
    try:
        with sync_playwright() as p:
            ...
            # Scrolluj w dół żeby załadować wszystkie oferty (lazy loading)
            prev_count = 0
            for _ in range(15):  # max 15 scrolli
                page.keyboard.press("End")
                time.sleep(1.5)
                count = page.evaluate("() => document.querySelectorAll('a[href*=\"/job-offer/\"]').length")
                if count == prev_count:
                    break  # nic nowego się nie załadowało
                prev_count = count

            print(f"[JustJoin] Zaladowano {prev_count} kart po scrollowaniu")
    ...
    print(f"[JustJoin] Pobrano {len(result_list)} pasujacych ofert")
    return result_list
